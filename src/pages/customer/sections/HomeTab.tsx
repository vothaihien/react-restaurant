...
const HomeTab: React.FC<HomeTabProps> = ({ onNavigate }) => {
    ...
    const loadMenus = async () => {
      ...
      try {
        ...
        notify({
          ...
          title: "Lỗi tải menu",
          description:
            error?.message || "Không thể tải danh sách menu. Vui lòng thử lại.",
        });
        setMenus([]);
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    ...
    const loadMenuDetail = async () => {
      ...
      try {
        const response = await menuApi.getChiTietMenu(
          selectedMenu.maMenu || selectedMenu.MaMenu
        );
        const data = response?.data || response;
        setMenuDetail(data);
      } catch (error: any) {
        notify({
          ...
          description:
            error?.message || "Không thể tải chi tiết menu. Vui lòng thử lại.",
        });
      } finally {
  ...
  const getImageUrl = (imageUrl: any) => {
    ...
    if (Array.isArray(imageUrl)) {
      url = imageUrl[0]?.URL || imageUrl[0]?.url || imageUrl[0];
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        {menuDetail.giaGoc &&
                          menuDetail.giaGoc >
                            (menuDetail.giaMenu || menuDetail.GiaMenu) && (
                            <span className="text-sm text-gray-400 line-through block">
                              {formatVND(
                                menuDetail.giaGoc || menuDetail.GiaGoc
                              )}
                            </span>
                          )}
                        <p className="text-2xl font-bold text-primary">
                          {formatVND(
                            menuDetail.giaMenu || menuDetail.GiaMenu || 0
                          )}
                        </p>
                    ...
                    {menuDetail.chiTietMenus &&
                      menuDetail.chiTietMenus.length > 0 && (
                        <div>
                          <p className="text-sm font-semibold text-gray-900 mb-3">
                            Danh sách món trong menu:
                          </p>
                          <div className="space-y-2">
                            {menuDetail.chiTietMenus.map(
                              (ct: any, index: number) => {
