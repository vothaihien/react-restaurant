...
const OrderModal: React.FC<OrderModalProps> = ({
  ...
  const { menuItems, categories, addItemsToTableOrder } =
      ...
      if (order && order.id) {
        try {
          ...
          if (data && data.monAns) {
            ...
            const rawItems: ExtendedOrderItem[] = itemsOfThisTable.map(
              (m: any) => {
                const originalItem = menuItems.find(
                  (mi: MenuItem) => mi.name === m.tenMon
                );
                return {
                  menuItem: {
  ...
  const handleSaveOrder = async () => {
    ...
    const newItemsToSend = currentOrderItems.filter(
    ...
    try {
      ...
      notify({
        tone: "error",
        title: "Lỗi",
        description: "Không thể lưu món ăn.",
      });
    }
  };
                  ...
                  >
                    ...
                    <div className="mt-2">
                      <input
                        ...
                        placeholder={
                          item.isConfirmed ? "Đã gửi bếp" : "Ghi chú bếp/bar"
                        }
